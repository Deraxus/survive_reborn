...
public class EventManager : MonoBehaviour
{
    ...
    // Событие вызывается, когда враг получает урон.
    // Передаёт объект врага и количество нанесённого урона.
    public event OnEnemyDamageDelegate OnEnemyDamageEvent;

    // Событие вызывается, когда враг умирает.
    // Передаёт объект умершего врага.
    public event OnEnemyDiedDelegate OnEnemyDiedEvent;

    // Событие вызывается примерно один раз в секунду.
    // Можно использовать для таймеров, периодических эффектов, регенерации и т.д.
    public event OnEverySecDelegate OnEverySecEvent;

    // Событие вызывается каждые 0.1 секунды.
    // Подходит для более частых проверок, эффектов, постепенного восстановления и т.п.
    public event OnEveryMicSecDelegate OnEveryMicSecEvent;

    // Событие вызывается при нажатии клавиши активации.
    // Клавиша берётся из InventoryController.activateKey.
    public event OnActivateKeyPressedDelegate OnActivateKeyPressedEvent;

    // Событие вызывается при покупке предмета.
    // Передаёт купленный объект item.
    public event OnItemBuyingDelegate OnItemBuyingEvent;

    // Событие вызывается при начале/факте перезарядки оружия.
    // Передаёт объект оружия.
